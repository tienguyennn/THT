...
jQuery(document).ready(($) => {
  ...
  $offcanvas
    .find(
      '[data-effect="drill"] ul.dropdown-menu, [data-effect="def"] ul.dropdown-menu',
    )
    .each((i, el) => {
      const $el = $(el);
      ...
      $el.prepend(
        "<span class='sub-menu-back'><i class='fas fa-chevron-left'></i>" +
          label +
          "</span>",
      );
  ...
  function initAccordionMenu($container) {
    const toggleSelector =
      ".sub-menu-toggle, .separator.dropdown-toggle, .nav-header.dropdown-toggle";
