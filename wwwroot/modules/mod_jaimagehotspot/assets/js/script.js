...
function addHammerJS(imgID, maxZoom, activeZ2CFD) {
  ...
  let CurrentTranX = 0,
    CurrentTranY = 0; // default will be center of image.
  let PrevTranX = 0,
    PrevTranY = 0;
  ...
  if (is_mobile_device()) {
    ...
    mc.on("pinch", function (ev) {
      ...
      // max range scale
      if (
        (currentScale <= 1 && currentScaleVal < 0) ||
        (currentScale >= maxZoom && currentScaleVal > 0)
      )
        return;
  ...
  if (!is_mobile_device() && !activeZ2CFD) return;
  ...
  mc.on("doubletap", function (ev) {
    ...
    if (currentScale == 1) {
      let WillScale = 2;
      CurrentTranX =
        (scaleIMG.width() / 2 - ev.srcEvent.layerX) * WillScale - 1;
      CurrentTranY =
        (scaleIMG.height() / 2 - ev.srcEvent.layerY) * WillScale - 1;
...
function centerObj(obj) {
  if (obj.length) {
    ...
    _top =
      jQuery(window).scrollTop() + (jQuery(window).height() - obj.height()) / 2;
    obj.css("top", _top + "px");
  }
}
  ...
  if ($e.find(".jashowvideo").length) {
    $e.find(".jashowvideo")
      ...
      .bind("click", function () {
        ...
        jQuery("body").append(
          '<div class="japopmedia' +
            idPOP +
            " japopover japopmedia webui-popover " +
            $elm.data("bgcolor") +
            ' touchdv popover fade in" style="position:fixed;z-index: 99999;width:' +
            vwidth +
            "px;max-width:" +
            vwidth +
            "px;height:" +
            ...
            jQuery(this).data("ifr") +
            "</div>" +
            "</div>",
        );
        jQuery(".japopmedia" + idPOP)
          .find(".popover-close")
          .off()
          .unbind()
          .bind("click", function () {
...
function OverAllData(_e, data, jaihp_settings) {
  ...
  var item_settings = {
    ...
    multi:
      jaihp_settings.trigger == "sticky" || jaihp_settings.multiple == 1
        ? true
        : false,
    animation: jaihp_settings.anim,
    onShow: function ($e) {
      if (is_mobile_device()) mobileCenter($e);
    },
    onHide: function ($e) {
  ...
  item_settings.url =
    rootURL +
    "index.php?option=com_ajax&module=jaimagehotspot&format=raw&method=getcontent&Itemid=" +
    menuID;
  item_settings.async = {
    type: "POST",
    data: data,
    ...
    success: function (that, data) {
      if (data.search("twitter") !== -1) {
        ...
        if (jQuery(".japopwarper_content").find("img").length) {
          ...
          else $img = jQuery(".japopwarper_content").find("img");
          $img.on("load", function () {
            ...
            $targetContent.removeAttr("style");
            that.displayContent();
            if (is_mobile_device()) mobileCenter(that.$target);
          });
        } else {
          var $targetContent = that.getContentElement();
  ...
  if (data.content_type == "social") {
    ...
    if (
      data.content_url.match("pinterest") ||
      data.content_url.match("instagram")
    ) {
      item_settings.width = data.popwidth == undefined ? 280 : data.popwidth;
      item_settings.height =
        data.popheight == undefined ? "auto" : data.popheight;
      if (is_mobile_device()) {
  ...
  if (data.content_type == "default") {
    ...
    if (Modernizr.touch && data.link) {
      item_settings.content +=
        '<br><a class="mobile-link" target="_blank" href="' +
        data.link +
        '">' +
        Joomla.JText._("JAI_MOBILE_POPUP_LINK") +
        "</a>";
    }
  }
...
jQuery(document).ready(function () {
  jQuery("body").prepend(
    '<div class="japopwarper_content" style="display:none;"></div>',
  );
