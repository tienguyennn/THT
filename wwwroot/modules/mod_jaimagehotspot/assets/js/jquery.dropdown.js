...
(function ($, window, undefined) {
  ...
  $.jaDropDown.prototype = {
    ...
    _layout: function () {
      ...
      this.size = {
        width: this.cloneElm.width(),
        height: this.cloneElm.height(),
      };
      ...
      if (Modernizr.csstransitions) {
        setTimeout(function () {
          self.opts.css(
            "transition",
            "all " + self.options.speed + "ms " + self.options.easing,
          );
        }, 25);
      }
    },
    ...
    _positionOpts: function (anim) {
      ...
      this.opts.each(function (i) {
        $(this).css({
          ...
          top: self.options.slidingIn
            ? (i + 1) * (self.size.height + self.options.gutter)
            : 0,
          left: 0,
          marginLeft: self.options.slidingIn
            ? i % 2 === 0
              ? self.options.slidingIn
              : -self.options.slidingIn
            : 0,
    ...
    open: function () {
      ...
      this.listopts.css(
        "height",
        (this.optsCount + 1) * (this.size.height + this.options.gutter),
      );
      this.opts.each(function (i) {
        $(this).css({
          opacity: 1,
          top: self.options.rotated
            ? self.size.height + self.options.gutter
            : (i + 1) * (self.size.height + self.options.gutter),
          ...
          transform: self.options.random
            ...
            : self.options.rotated
              ...
              : "none",
          transitionDelay:
            self.options.delay && Modernizr.csstransitions
              ? self.options.slidingIn
                ? i * self.options.delay + "ms"
                : (self.optsCount - 1 - i) * self.options.delay + "ms"
              : 0,
        });
      });
      this.opened = true;
    ...
    close: function () {
      ...
      if (this.options.delay && Modernizr.csstransitions) {
        this.opts.each(function (i) {
          $(this).css({
            "transition-delay": self.options.slidingIn
              ? (self.optsCount - 1 - i) * self.options.delay + "ms"
              : i * self.options.delay + "ms",
          });
        });
